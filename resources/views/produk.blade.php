    ...
    @foreach ($posts->skip(1) as $post)
      <div class="col-md-3 mb-3 mt-4">
        <div class="card card-produk" >
          {{-- <div class="position-absolute px-3 py-2 " style="background-color:rgba(0, 0, 0, 0.4)"><a class="text-white text-decoration-none" href="/posts?category={{ $post->category->slug }}">{{$post->category->name}}</a></div> --}}
          @if($post->image)
          ...
          @endif
          {{-- <img class="img-produk" src="https://source.unsplash.com/500x400?{{$post->category->name}}" class="card-img-top" alt="{{$post->category->name}}"> --}}
          <div class="card-body">
            <h5 class="card-title">{{$post->title}}</h5>
            <p>
              <small class="text-muted">
                Review oleh <a href="/posts?author={{$post->author->username}}" class="text-decoration-none">{{$post->author->name}}</a>
                <br>
                Jenis Produk <a href="/posts?author={{$post->category->name}}" class="text-decoration-none">{{$post->category->name}}</a> 
              </small>
            </p>
            <p class="card-text">{{$post->excerpt}}</p>
          </div>
          <div class="link-produk">
            <a href="posts/{{$post->slug}}" class="btn btn-primary btn-produk">Detail Produk</a>
          </div>
        </div>
      </div>
    @endforeach
